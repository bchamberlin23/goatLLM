...
#[tauri::command]
fn memory_list(category: Option<String>, state: tauri::State<'_, DbState>) -> Result<Vec<MemoryRow>, String> {
    let db = state.db.lock().map_err(|e| format!("Lock error: {}", e))?;
    
    if let Some(cat) = category {
        let mut s = db.prepare("SELECT id, text, category, uses, created_at FROM memories WHERE category = ?1 ORDER BY created_at DESC")
            .map_err(|e| e.to_string())?;
        let rows = s.query_map(rusqlite::params![cat], |row| {
            Ok(MemoryRow {
                id: row.get(0)?,
                text: row.get(1)?,
                category: row.get(2)?,
...
#[tauri::command]
fn memory_delete(id: String, state: tauri::State<'_, DbState>) -> Result<(), String> {
    let db = state.db.lock().map_err(|e| format!("Lock error: {}", e))?;
...
#[tauri::command]
fn memory_increment_uses(id: String, state: tauri::State<'_, DbState>) -> Result<(), String> {
    let db = state.db.lock().map_err(|e| format!("Lock error: {}", e))?;
    db.execute("UPDATE memories SET uses = uses + 1 WHERE id = ?1", rusqlite::params![id])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn run() {
    let app_handle = tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            searxng::searxng_install_docker,
            searxng::searxng_start_docker_daemon,
        ])
