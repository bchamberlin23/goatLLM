...
async function scrapeWithFirecrawl(
  ...
  return { ...scraped, content: truncateContent(scraped.content, options.maxChars) };
}
...
export async function scrapeUrl(url: string, options: ScrapeOptions = {}): Promise<ScrapedPage> {
  ...
  if (apiKey) {
    try {
      return await scrapeWithFirecrawl(url, apiKey, options);
    } catch {
      // Fall through to the local, SSRF-protected reader so a Firecrawl
      // outage or a provider-side page rejection does not lose the source.
    }
  }

  const fallback = await browserFetch({ url, mode: "text", timeoutMs: options.timeoutMs ?? 15_000 });
  const content = truncateContent(fallback.content, options.maxChars);
  return {
