...
export function FollowUpActions({ fileInputRef, onFilesChange, showMic, speechListening, onToggleMic, designMode, activeId, agentMode, planMode, onDisablePlanMode }: FollowUpActionsProps) {
  return (
    <>
      ...
      {showMic && (
        <button onClick={onToggleMic} className={["control-icon p-1.5 rounded-md transition-colors", speechListening ? "text-accent bg-accent/10 border-accent/25" : ""].join(" ")} aria-label={speechListening ? "Stop dictation" : "Start dictation"} aria-pressed={speechListening} title={speechListening ? "Stop listening" : "Dictate"} type="button">
      ...
      {agentMode && planMode && (
        <button type="button" onClick={onDisablePlanMode} title="Plan mode — read-only investigation. Click to turn off." className="flex items-center gap-1.5 px-2.5 h-7 rounded-full bg-accent/10 hover:bg-accent/15 border border-accent/30 text-[12px] font-medium text-accent transition-colors shrink-0">
