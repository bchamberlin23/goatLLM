      ...
      {showChips && displayedSkills.length > 0 && (
        <div className="mb-2 flex items-center gap-1.5 flex-wrap">
          {displayedSkills.map((name) => (
            <span key={name} className="inline-flex items-center gap-1.5 pl-1.5 pr-1 py-0.5 rounded-full bg-accent/10 border border-accent/25 text-[12px] text-[#d4944a] shadow-[inset_0_1px_0_rgba(var(--theme-accent-rgb),0.08)]">
        ...
        <>
          ...
          <div className="popover-surface motion-popover-in absolute bottom-full left-0 mb-2 w-72 rounded-xl p-1.5 z-[90] origin-bottom-left">
            ...
            {skillsForCurrentMode.filter((skill) => !disabledSkills.has(skill.name)).map((skill) => {
              ...
              const modeColor = skill.mode === "agent" ? "text-[#d4944a] bg-accent/10 border-accent/20" : skill.mode === "chat" ? "text-[#7eb8f7] bg-[#3b82f6]/10 border-[#3b82f6]/20" : "text-[#b4a0f7] bg-[#8b5cf6]/10 border-[#8b5cf6]/20";
              return (
