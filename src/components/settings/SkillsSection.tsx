...
export function SkillsSection({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={`flex flex-col gap-3 ${embedded ? "pt-2 border-t border-white/5" : ""}`}>
          ...
          {discoveredSkills.length > 0 ? (
            <div className="flex items-center gap-1.5">
              ...
              {autoCount > 0 && (
                <>
                  <span className="text-[10px] text-text-3">·</span>
                  <span className="inline-flex items-center gap-1 text-[11px] text-[#d4944a]">
      ...
      {discoveredSkills.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-6 px-4 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                  ...
                  className={`flex flex-col gap-0 rounded-xl border transition-all duration-150 ${
                    isAutoTrigger && !isDisabled
                      ...
                      : isDisabled
                        ...
                        : "bg-[#212122] border-white/[0.07] hover:border-white/[0.12] hover:bg-[#232324]"
                  }`}
                >
                  {/* Main row */}
                  <div className="flex items-center justify-between gap-3 px-3 py-2.5">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isAutoTrigger && !isDisabled && (
                          <span className="shrink-0 text-[9px] text-[#d4944a] bg-accent/10 border border-accent/20 px-1.5 py-0.5 rounded flex items-center gap-1 font-medium leading-none">
                      ...
                      <div
                        className="text-[11px] text-[#a0a0a0] truncate mt-0.5 leading-relaxed"
                      ...
                      <div className="text-[10px] text-[#666] mt-0.5 font-mono">{skill.source}</div>
                    </div>
                    {/* Enable toggle */}
                    <button
                  ...
                  <div className={`flex items-center justify-between px-3 pb-2.5 ${isDisabled ? "pointer-events-none" : ""}`}>
                    <div className="flex items-center gap-1.5">
                      <Zap size={10} className={isAutoTrigger && !isDisabled ? "text-[#d4944a]" : "text-text-3"} />
      ...
      <div className="mt-1">
        ...
        <div className="flex gap-2 mb-2">
          <input
            ...
            className="flex-1 bg-[#1a1a1c] border border-white/[0.07] rounded-lg px-3 py-1.5 text-[13px] text-[#d5d5d5] placeholder:text-[#555] focus:outline-none focus:border-white/[0.14] transition-colors font-mono text-[12px]"
          />
          <button
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.06] hover:bg-white/[0.10] border border-white/[0.07] text-text-2 text-[12px] font-medium rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
        ...
        {skillPaths.length > 0 && (
          <div className="flex flex-col gap-1">
            {skillPaths.map((p) => (
              <div
                key={p}
                className="group flex items-center justify-between gap-2 px-3 py-2 bg-[#1e1e20] border border-white/[0.06] rounded-lg hover:border-white/[0.10] transition-colors"
              >
                ...
                <button
                  className="shrink-0 w-5 h-5 flex items-center justify-center rounded text-text-3 hover:text-[#f87171] hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100"
...
function CreateSkillForm({
  ...
  const modeOptions: { value: "chat" | "agent" | "both"; label: string; color: string }[] = [
    ...
    { value: "agent", label: "agent", color: "text-[#d4944a] border-[#f59e42]/40 bg-[#f59e42]/15" },
    { value: "both", label: "both", color: "text-[#b4a0f7] border-[#8b5cf6]/40 bg-[#8b5cf6]/15" },
  ];

  return (
    <div className="flex flex-col gap-3 p-3.5 bg-[#1a1a1c] border border-white/[0.08] rounded-xl">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-[#a0a0a0] leading-relaxed mt-0.5">
            ...
            <code className="text-[#c9c9c9] bg-white/5 px-1 py-0.5 rounded">
      ...
      {showHelp && (
        <div className="flex flex-col gap-2 p-3 bg-[#212122] border border-white/[0.06] rounded-lg text-[11.5px] text-[#b4b4b4] leading-relaxed">
          ...
          <p><strong className="text-[#ececec]">Body</strong> — the actual instructions. Use plain markdown. Address the model directly: &ldquo;You answer in plain prose&hellip;&rdquo;. Examples beat abstractions.</p>
          <p className="text-[#888]">Tip: keep it under ~30 lines. Long skills crowd the model&rsquo;s attention.</p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          maxLength={80}
          className="bg-[#161618] border border-white/[0.08] rounded-lg px-3 py-1.5 text-[13px] text-[#d5d5d5] placeholder:text-[#4a4a4c] focus:outline-none focus:border-white/[0.16] transition-colors"
        />
        {name && slug !== name && (
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          rows={2}
          className="bg-[#161618] border border-white/[0.08] rounded-lg px-3 py-1.5 text-[13px] text-[#d5d5d5] placeholder:text-[#4a4a4c] focus:outline-none focus:border-white/[0.16] transition-colors resize-none"
        />
      </div>
          ...
          {modeOptions.map((opt) => (
            <button
              ...
              className={`flex-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all border ${
                mode === opt.value
                  ? opt.color
                  : "bg-white/[0.03] text-[#888] border-white/[0.06] hover:text-[#ececec] hover:bg-white/[0.06] hover:border-white/[0.10]"
              }`}
            >
              {opt.label}
            </button>
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          rows={8}
          className="bg-[#161618] border border-white/[0.08] rounded-lg px-3 py-2 text-[13px] text-[#d5d5d5] placeholder:text-[#4a4a4c] focus:outline-none focus:border-white/[0.16] transition-colors font-mono resize-y leading-relaxed"
        />
      </div>
